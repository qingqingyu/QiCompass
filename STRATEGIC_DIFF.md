...
> **范围**：本次 grill 锁定的战略决策对**当前代码**产生的 diff。每项标 `[ ] TODO` 表示未动手。
...
- [ ] **改 default Tab**：`@State private var selectedTab: Tab = .deepAnalysis` → `.dailyFortune`（在 `selectedTab` 属性声明处）
- [ ] **加第 4 个 Tab `.profile`**：
...
- [ ] **onboarding sheet 后接 BirthFormView**：当前 `onComplete` 回调只设 `hasSeenOnboarding = true`。改为：先呈现 `BirthFormView` 作为 fullScreenCover，表单提交成功后才 dismiss onboarding + 落地 TabView（默认今日运势）
- [ ] **`switchTab` Notification 加 `.profile` case**：在 `switchTab` 方法的 switch 里加 `case Tab.profile.switchKey: selectedTab = .profile`

#### `ios/QiCompass/QiCompass/Features/Onboarding/OnboardingView.swift`

- [ ] **StartPage CTA 改向**：当前 `onComplete()` 触发后 `hasSeenOnboarding = true` 直接 dismiss。改为触发后**进入 BirthFormView**（不 dismiss onboarding，而是 push/navigate to form）。表单完成才完整 dismiss。
- [ ] **保留现有 4 页设计**（WelcomePage / StancePage / PrivacyPage / StartPage 不动，2026-08-01 决策#18）

#### `ios/QiCompass/QiCompass/Features/DeepAnalysis/BirthFormView.swift`

- [ ] **加 first-launch 模式**：现有 BirthFormView 用于"深度解析 Tab 内排盘"。新增"首次注册"模式（B2 流）：
...
- [ ] **不立刻跑深度解析 AI**（B2 核心）：表单提交只触发 `/api/bazi/calculate` 排盘 + chart 存档，**不**触发 `/api/interpret bazi_deep_free`

### 深度解析模块

#### `ios/QiCompass/QiCompass/Features/DeepAnalysis/DeepAnalysisView.swift`

- [ ] **加 chart 顶部 deterministic anchor sentence**：pillars 表上方加 1 行文本，由后端 `day_master_strength` + `favorable_elements` + `unfavorable_elements` 拼接返回，instant，0 AI 成本。文案模板待定（候选："你的日主是 **{day_gan}**（{day_gan_element}），命局整体 **{day_master_strength_label}**，**喜 {favorable_elements}**、**忌 {unfavorable_elements}**。"）
- [ ] **改"生成免费解读"按钮为触发点（β）**：当前流程如果是 auto-generate，改为先显示按钮 + disabled 状态。用户点击 → 触发 ViewModel 的 `generateFreeChapters()` → ~10s skeleton loading → fade in
- [ ] **移除"再生成"按钮**（如果有）：任何模块都不放重跑按钮

#### `ios/QiCompass/QiCompass/Features/DeepAnalysis/DeepAnalysisViewModel.swift`

- [ ] **拆 `loadOrCreateChart()` 与 `generateFreeChapters()`**：
...
- [ ] **加 `freeChaptersState: GenerationState`**：`.idle` / `.generating` / `.success` / `.failed`。View 根据状态显示按钮 / skeleton / 内容 / 错误重试

#### `ios/QiCompass/QiCompass/Features/DeepAnalysis/PaidChaptersLockView.swift`

- [ ] **检查现有付费章节锁定 UI**：确认是否已实现 5 章付费锁标。如果没有，按 MONETIZATION.md §免费/付费内容分界 实现

#### `ios/QiCompass/QiCompass/Features/DeepAnalysis/InterpretationSection.swift`

- [ ] **章节渲染适配 Medium-deep voice**：每章渲染时按段落（3-5 段）+ 短句呈现。如果现有实现是单段长文，需调整渲染逻辑

### 今日运势模块

#### `ios/QiCompass/QiCompass/Features/DailyFortune/DailyFortuneView.swift` + 子组件

- [ ] **AI 总览渲染压到 50-80 字**：当前实现假设 150-200 字 prompt 输出。需要：
  - 调整 Text frame / lineLimit 容纳 50-80 字
  - 排版节奏：3-5 短句，可以一行一句或自然换行
- [ ] **宜/忌升为主视觉锚点**：当前宜/忌可能是次要 chip 列表。改为视觉 prominent block（如：左"宜"列 + 右"忌"列，每列 2-3 字 bullet）。视觉规范：朱砂/墨青做宜/忌区分，对齐 DESIGN.md 五行色映射
- [ ] **12 时辰保持默认折叠**：当前已经是默认折叠，确认不动
- [ ] **移除"再生成"按钮**（如果有）

#### `ios/QiCompass/QiCompass/Features/DailyFortune/DailyFortuneViewModel.swift`

- [ ] **流式追加支持**（决策 #14 配套，参 Open Question #20 已关闭）：AI 总览生成期间（3-5s），UI 显示 skeleton / 进度提示。后端如果支持 SSE 流式返回，前端 streaming 渲染；否则 polling 或 single-shot + loading state

### 合盘模块

#### `ios/QiCompass/QiCompass/Features/Compatibility/*`

- [ ] **章节渲染适配 Medium voice**：与深度解析同样模式（200-300 字/章 + 3-5 段 + 短句）
- [ ] **检查现有付费章节锁定 UI**：与深度解析同形态（免费基础 + 付费深度），按 MONETIZATION.md §合盘 实现
- [ ] **移除"再生成"按钮**（如果有）
...
- [ ] **重写 5 个模板的 voice 段落**（对齐 `bazi-app-design-doc.md` §AI Voice 规范）。**当前代码 vs 目标规范对照**：
...
- [ ] **bump `PROMPT_VERSIONS`**：5 个 module 全部 +1（bazi_deep 1→2, bazi_deep_free 1→2, bazi_deep_paid 1→2, compatibility 1→2, daily_fortune 1→2）。**这是关键**：bump 后老 iOS 客户端的本地缓存（按 prompt_version 隔离）自动失效，强制重新生成新 voice 内容
- [ ] **注意 breaking change for 老 TestFlight 用户**：B2 流改入口 + β 点击触发 + prompt_version bump 三重叠加 → 老用户升级后会经历：(1) 深度解析缓存失效（prompt v1 → v2）；(2) 深度解析 Tab 不再 auto-generate，而是显示"生成免费解读"按钮。**desired behavior**（符合 grill 决策），但需要在 release notes 或首次升级引导里告知

#### `backend/app/api/interpret.py`（或对应路由层）

- [ ] **检查 chart anchor sentence 接口**：决策 #11 要求深度解析 Tab 顶部有 deterministic anchor。需要：
...
### 预先存在的 doc drift（**不在本次 grill 范围，flag 待清理**）

#### `bazi-app-design-doc.md` §Visual Design Tokens

- [ ] **TODO（不在本次范围）**：line ~488-502 还是旧黑金色板（bgTop `#0d0b08` / bgMid `#12100d` / 主金 `#c9a03c` / 亮金 `#f5d785` 等），与 DESIGN.md 的 paper `#FDFCFA` / cinnabar `#C33B3B` / jade `#2C5F3F` 决策**直接矛盾**
- 处理建议：下次视觉相关 slice 起手时，删除整个 §Visual Design Tokens 段落，改为引用 `DESIGN.md §Color`

#### `bazi-app-design-doc.md` §V1 Minimum Viable Aesthetic

- [ ] **TODO（不在本次范围）**：line ~678+ 写"金底深色（不可协商）+ ZCOOL XiaoWei 显示字体"，与 DESIGN.md "极浅暖白 paper + Songti SC" 决策**直接矛盾**
- 处理建议：同上，下次视觉 slice 起手时删除或重写

---

## 验收标准（本次 grill 决策落地的判定）

- [ ] 冷启动 App → 落地默认 Tab = 今日运势（不是深度解析）
- [ ] Tab bar 显示 4 个：今日运势 / 深度解析 / 合盘 / 我的
- [ ] 首次启动 → OnboardingView 4 页 → StartPage CTA → BirthFormView → 表单提交 → 落地今日运势（已个性化，不跑深度解析 AI）
- [ ] 第二次冷启动 → 直接到今日运势（跳过 OnboardingView）
- [ ] 切到深度解析 Tab → chart 立即可见 + anchor sentence → "生成免费解读"按钮 → 点击 → ~10s → 2 章 fade in
- [ ] 今日页 AI 总览篇幅 ≤ 80 字（实测）
- [ ] 深度章节每章 200-300 字（实测），分 3-5 段，每段 2-3 短句
- [ ] 合盘页章节同 Medium voice（实测）
- [ ] 任何模块都**没有**"再生成"按钮
- [ ] `prompts.py` 的 `PROMPT_VERSIONS` 5 个 module 全部 = 2
- [ ] 老缓存（prompt_version=1）自动失效，重新生成新 voice
- [ ] 用真实命盘 spike：跑 20 个盘，长句（> 50 字/句）占比 < 20%

---

## Open implementation questions（战略已锁，但实施细节待定）

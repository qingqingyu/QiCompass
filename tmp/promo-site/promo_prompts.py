...
篇幅(激进 ~5x):
- bazi_deep_free / _paid:每章 1000-1500 字
  (2026-08-14 加「第一章:十神结构与命局主线」——free 3 章 / paid 6 章,
  承接 v1 设计文档 M0 的十神结构叙事;数据用 HEADER 已注入的四柱十神+藏干)
...
# 加长版调用参数:bazi paid 6 章(含十神结构章)上限 9000 字 → 24576 token
# 上限 + 900s 超时(2026-08-14 从 16384 上调 token:6 章 × 1500 字 ≈ 13.5k-18k
# token,16384 会截断;2026-08-15 超时 420s→900s:非流式调用要整篇生成完才返回,
# z.ai 中转输出速度波动,9k+ token 慢时要 >7 分钟,实测 421s ReadTimeout)。
# App 路径仍是 config 默认 1024 / 90s,不受影响。
PROMO_MAX_OUTPUT_TOKENS = 24576
...
BAZI_DEEP_PAID_PROMO_REQUIREMENTS = """写作要求（付费 6 章 · 长文版，每章 1000-1500 字，总 6000-9000 字）：

""" + _BAZI_TEN_GOD_CHAPTER + """**第二章：财运**（1000-1500 字）
正财 / 偏财倾向、求财方式、富贵层级、财星触发的流年窗口、财务上的风险点。
关键年份点出来（结合大运流年），给可操作的求财建议。

**第三章：爱情**（1000-1500 字）
婚姻方向、配偶特征（配偶星 + 配偶宫）、婚期窗口、相处模式、需要磨合的点。
已有定论的写倾向，不确定的诚实说"需结合对方命盘"。

**第四章：健康**（1000-1500 字）
体质倾向（五行偏盛偏衰对应）、需要注意的部位、调候建议、作息与运动方向。
写到具体可执行，不停留在"注意身体"。

**第五章：六亲**（1000-1500 字）
父母缘 / 兄弟缘 / 子女缘，各给一段以上的篇幅，每段落到盘面依据
（某宫位、某十神与日主的关系），并给相处建议。

**第六章：晚年**（1000-1500 字）
晚运倾向、中年转折点（哪步大运换轨）、晚年的调候与安顿建议。
时间窗口写具体（如"×运后十年"），不空泛。

...
5. **后端合盘契约**:「零后端改动」的前提是**发起前拦截**(第 2 条)——无时辰对永不发起,请求永不携带无时辰盘,契约无需表达该状态。注意现状契约**并非**「无必填时辰假设」,而是有两处硬假设构成天然 422 防线,如实记录两处硬假设:模式 A `ChartPayload.day_master_strength` Literal 与 `four_pillars` 必含 `hour`(`app/models/daily_fortune.py:44,46`);模式 B `PersonBInput` 无 `hour_known` 字段(`app/models/compatibility.py:33`)。

   **2026-09-01 review 更正**:模式 A 的两处**已由 S01 扩掉**(它同时是每日运势的请求模型,S09 必需),因此它**不再是合盘的 422 防线**——本 slice 的发起前拦截从「契约兜底」变成**唯一防线**,必须真正拦住,不能依赖后端报错。模式 B(`PersonBInput`)仍无 `hour_known`,保持不动。若未来放开对级拦截(允许无时辰合盘),需给模式 B 加字段 + 补一份 `compatibility_free` 独立字段集(现状 free/paid 共用同一份 REQUIRED,含 `hour_a`/`hour_b`),本期不做
...
- [ ] **发起前拦截为唯一防线**:模拟绕过 UI 直接发起无时辰对 → 不应依赖后端 422 兜底(S01 已放开模式 A Literal),VM 层必须先拦

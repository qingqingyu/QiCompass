...
**2026-09-01 review 回填**(5 处):S01 补 `ChartPayload` 契约(原分割无主,S09 依赖它)/ S02 日柱判据改 D3 区间测试(原「offset < −60」单边近似两个方向都错)/ S05 补 `ChartPayloadDTO.from` 与 `PromptContextBuilder:234` 锚点 / S06 补 en 路径(照抄 special_pattern 先例会让英文用户 raise)/ S07 补每日运势一期临时全拦(原一期无人覆盖默认落地 Tab)。Parent 决策文档已同步。

## 实施顺序

| Slice | 标题 | 类型 | Blocked by | 一句话 |
|---|---|---|---|---|
| S01 | [后端 hour_known 契约 + 三柱排盘 + 喜忌 unknown_hour(tracer bullet)](S01-后端hour_known契约与三柱排盘.md) | AFK | 无 | API 收 `hour_known=false` → 12:00 占位、`pillars.hour=null`、strength=unknown_hour、喜忌留空、深夜日柱歧义;**并扩 `ChartPayload` 契约**(每日运势/合盘共用模型,不扩则无时辰盘连每日运势请求都发不出);老客户端零变化 |
| S02 | [D10 节气边界双排盘比对](S02-D10节气边界双排盘比对.md) | AFK | S01 | 00:00/23:59 探针只比对**年/月柱**(日柱走 D3 区间测试,不走探针);立春→生肖未知;节交界→调候/得令/大运序列不可用;日柱歧义→D5 全拦截分支 |
...
| S07 | [iOS 一期拦截临时态(付费墙 + 每日运势)](S07-iOS付费墙拦截临时态.md) | AFK | S01、S04、S05、S06 | 付费 8 章临时全拦 + 合盘任一方无时辰整对拦(免费亦拦,free/paid 共用同一份 REQUIRED)+ **每日运势一期临时全拦**(默认落地 Tab,一期不能留空白):不展示价格、不触发 purchase、「补充出生时刻后解锁」 |
...
每期可独立验收;一期结束 = 无时辰用户可以诚实建档、看到三柱盘 + 免费 2 章降级叙事、付费被拦、**每日运势(默认落地 Tab)有明确拦截终态而非白屏/422**、生肖屏安全;二期结束 = 每日运势降级、付费墙拦截页文案适配(S10 落地,S07 临时态 → 正式版)、补时辰闭环、合盘标记。
...
- **共用模型**:`ChartPayload`(`app/models/daily_fortune.py`)被**每日运势与合盘共用**,`DailyFortuneRequest.chart_payload` 与 `chart_payload_a/b` 同一个类——动它要同时想两个模块;iOS 对应 `ChartPayloadDTO.from`(`DailyFortuneOrchestrator.swift:355-372`),现有 `?? "special_pattern"` 静默兜底必须显式处理,`unknown_hour` 落进去会伪装成从格盘
- **日柱歧义判据单一事实源**:Parent D3 的三步区间测试(候选墙钟区间 + offset → 是否含真太阳时 23:00 点)。**任何 slice 不得另立阈值规则**——单边近似在东偏经度漏报、西偏经度过度降级
- **prompt 语言维度**:`render_prompt(module, context, language)` 按语言加载模板,`prompts/en/` 是活目录;新增任何条件 suffix 必须 zh/en 同时可渲染,不得沿用 special_pattern 的 `raise FileNotFoundError` 占位

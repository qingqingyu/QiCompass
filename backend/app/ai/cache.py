...
- model 列非 PK,记录每条解读由哪个 model 生成(观测/排查用)
- 模型升级时必须 bump prompt_version 使老缓存失效;model 列用于事后排查

错误显式传播(严格遵守 CLAUDE.md):
- sqlite3 异常不吞,向上抛 → 路由层包成 InterpretationCacheError(500)
- cache get 失败 → 抛 500(不降级为 Claude 调用,避免缓存层故障被静默掩盖)
...
CREATE TABLE IF NOT EXISTS interpretation_cache (
    ...
    PRIMARY KEY (content_hash, module, prompt_version, target_date, prompt_hash)
);
"""
...
class InterpretationCache:
    ...
    def get(self, *, content_hash: str, module: str,
            ...
            prompt_hash: str) -> dict[str, Any] | None:
        """查缓存。
        ...
        Returns:
            命中 → dict(model, interpretation, generated_at)
            未命中 → None
        ...
        with self._connect() as conn:
            ...
            row = conn.execute(
                "SELECT model, interpretation, generated_at "
    ...
    def set(self, *, content_hash: str, module: str, prompt_version: int,
            target_date: str | None, prompt_hash: str, model: str,
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                " model, interpretation, generated_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (content_hash, module, prompt_version, td, prompt_hash,
                 model, interpretation, generated_at),
            )
...
_EXPECTED_COLUMNS = frozenset({
    ...
    "prompt_hash", "model", "interpretation", "generated_at",
})

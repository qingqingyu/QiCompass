...
PROMPT_VERSIONS: dict[str, int] = {
    ...
    "bazi_deep_paid": 4,   # M2 拆分:9 章付费(2026-08-15 加天赋/高低配/系统模式三章,3→4)
...
# M2 拆分:付费 9 章(需 entitlement 才能调用)
# 章节:自我认知篇 1.十神结构 2.天赋能力 3.高配低配 4.人生系统模式
#       生活领域篇 5.财运 6.爱情 7.健康 8.六亲 9.晚年
# 具体领域预测是用户付费动力(MONETIZATION.md §决策汇总)
# 2026-08-01 grill-me V2:voice 改 Medium-deep(短句节奏 + 每段聚焦一个洞察)
# 2026-08-14 加第一章「十神结构与命局主线」(承接 v1 设计文档 M0 叙事;
# 数据用 HEADER 已注入的四柱十神+藏干,prompt_version 2→3)
# 2026-08-15 加第二~四章 v1 结构叙事精华(M1 天赋/M2 高低配/M3 系统模式,
# 压缩为 200-300 字章;完整 1500-2500 字版仍归 v1 链独占),prompt_version 3→4
BAZI_DEEP_PAID_TEMPLATE = _BAZI_DEEP_HEADER + """
写作要求（付费 9 章，每章 200-300 字，总 1800-2700 字）：

**第一章：十神结构与命局主线**（200-300 字）
十神主线三层（主导 / 次要 / 潜伏，点名柱位与藏干依据）+ 核心循环（哪两个十神 A→B→A，靠什么驱动、在哪里损耗）。分 3-5 段，每段 2-3 短句。
第一段用一句「命局呈现××结构倾向」概括运转方式（不给格局硬分类），末段点出这个结构如何引出后面各章。

**第二章：天赋能力**（200-300 字）
最自然、不费力的能力 2-3 项（写具体行为场景，不写形容词）+ 区分哪些是结构天生、哪些是环境训练出来的（点明依据十神与柱位）+ 末段一句话点出真正的杠杆能力。分 3-5 段，每段 2-3 短句。

**第三章：高配版与低配版**（200-300 字）
同一个结构的两种跑法。高配版行为画像（2-3 个外部可观察信号）+ 低配版行为画像 + 一个核心分界变量（环境/信念/觉察中选最关键的一个）+ 2-3 个低配化早期信号（日常可自查）。不写成命定的两条路。分 3-5 段，每段 2-3 短句。

**第四章：人生系统模式**（200-300 字）
把命局当系统：输入什么、怎么加工、输出什么、靠什么回血（对齐第一章的核心循环）+ 最怕的环境（写失效机制，不写"不适合"三个字）+ 适合的生活结构（时间节奏 / 协作密度 / 反馈周期，点一两个最关键的）。分 3-5 段，每段 2-3 短句。

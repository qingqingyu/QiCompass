"""evalkit 20 盘固定用例(迁自 spikes/prompt_validation/fixtures.py,原样)。
...
Q5 run 身份:CASES_HASH 是对 20 盘规格的规范化 JSON 求 sha256,
用例集合任何变动 → hash 变 → 新 run,不与老 run 混比。
"""
...
def _cases_hash() -> str:
    """对 20 盘规格的规范化 JSON 求 sha256(sort_keys 消除 dict 顺序影响)。"""
    canonical = json.dumps(CASES, ensure_ascii=False, sort_keys=True,
                           separators=(",", ":"))
    return hashlib.sha256(canonical.encode("utf-8")).hexdigest()


CASES_HASH: str = _cases_hash()

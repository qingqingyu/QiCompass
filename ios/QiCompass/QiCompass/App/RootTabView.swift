...
///
/// 包装 BirthFormView + 状态观察。提交成功(vm.state → .chartReady)时调用 onComplete,
...
private struct FirstLaunchBirthFormView: View {
    ...
    /// 防止 chartReady → onAppear 多次触发 onComplete(SwiftUI 重渲染时 onAppear 可重复调用)。
                ...
                Color.clear
                    .onAppear {
                        guard !hasTriggeredComplete else {
                            AppLogger.app.warning("FirstLaunchBirthFormView state=chartReady onAppear 已触发过 onComplete,跳过重复调用")
                            return
                        }
                        hasTriggeredComplete = true
                        AppLogger.app.info("FirstLaunchBirthFormView state=chartReady → 触发 onComplete")
                        onComplete()
                    }
